...
`deploy-release.sh` accepts only the atomic commit-named artifact-set directory and requires exactly its archive and sidecar. Under the shared lock it cleans safe abandoned staging, reserves the compressed cap plus fixed upload margin before copying either file, copies both into persistent root-private new inodes, verifies SHA-256, enforces the documented compressed, member-count, per-member, expanded-size, filesystem-metadata, inode, and free-space caps, rejects unsafe archive entries, and validates the manifest, image digest, helper-journal compatibility, built CLI, dependencies, and overlay. The local packager enforces the artifact limits before publication. Its pure-Bash ref validator accepts the ordinary subset emitted by the packager without installing or running Git on production. Candidate copies of the systemd unit, Caddyfile, and drop-in must be byte-identical to the installed managed control plane. The helper publishes and activates code only and owns locked atomic invite/session rotation with rollback; it never runs candidate commands or updates the stable helper, unit, or Caddy files.

The helper owns separate versioned format-1 activation and rotation journals and rejects unknown formats. Reconciliation precedes every operation: prepared rotation restores, syncs, restarts, and accepts the old environment; committed rotation keeps the new value and cleans state; failed recovery retains state and blocks later work. Rotation requires a non-empty validated active release before publishing its journal.
