...
The server installs only the Node.js 24 runtime and Caddy; it has no builder account, pnpm, source checkout, dependency lifecycle execution, test runner, or build cache. On the development machine, `package-release.sh` archives an exact named Git ref and runs frozen dependency installation, the invite-auth tests, the full build, and config dump inside a resource-bounded ephemeral official Node 24 Linux container with fresh state and pinned pnpm tarball integrity. The resulting deterministic archive contains the complete Linux runtime tree, dependencies, built frontend and libraries, deployment data, and a manifest with format, commit, named ref, platform, Node, pnpm, and helper-journal compatibility.

The stable host helper copies the artifact and checksum into root-private new inodes, verifies SHA-256, rejects absolute paths, parent traversal, special files, duplicate entries, and escaping links, extracts without preserving uploaded ownership, validates the manifest and required outputs, and publishes the root-owned immutable commit directory. It never runs candidate Git, package management, lifecycle hooks, tests, build commands, config scripts, or release-contained helpers. systemd runs the accepted release as the non-login `mydsh` runtime user from `/srv/mydsh/workspace`; Caddy reads only the public environment file.
