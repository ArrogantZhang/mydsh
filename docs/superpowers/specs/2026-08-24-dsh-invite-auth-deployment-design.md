...
The Linux amd64 server installs only the Node.js 24 runtime and Caddy; bootstrap rejects any other `dpkg` architecture before locking or mutation, and the host has no builder account, pnpm, source checkout, dependency lifecycle execution, test runner, or build cache. On the development machine, `package-release.sh` archives an exact named Git ref and runs frozen dependency installation, the invite-auth tests, the full build, and config dump inside a resource-bounded ephemeral official Node 24 Linux container with fresh state and pinned pnpm tarball integrity. The container sees only the exact source tree and an unpredictable mode-0700 artifact-set staging directory, never the caller's output directory. After the container exits, the host validates and syncs the tarball and checksum, then atomically renames their single commit-named parent directory into the output. The deterministic archive contains the complete Linux amd64 runtime tree, dependencies, built frontend and libraries, deployment data, and a manifest with format, commit, named ref, platform, Node, pnpm, and helper-journal compatibility.

The stable host helper requires the atomic artifact-set directory to contain exactly the tarball and checksum, copies both into root-private new inodes, verifies SHA-256, rejects absolute paths, parent traversal, special files, duplicate entries, and escaping links, extracts without preserving uploaded ownership, validates the manifest and required outputs, and publishes the root-owned immutable commit directory. Before activation it also enforces every security-relevant systemd value exactly once, including the runtime identity, working directory, environment files, command, restart policy, mask, hardening, writable paths, and install target. It never runs candidate Git, package management, lifecycle hooks, tests, build commands, config scripts, or release-contained helpers. systemd runs the accepted release as the non-login `mydsh` runtime user from `/srv/mydsh/workspace`; Caddy reads only the public environment file.
...
The root-installed helper builds a complete format-1 `prepared` journal in a root-only `activation.new.*` sibling containing the previous link, host-file backups, and prior service enablement, fsyncs it, and atomically renames it to `activation` before installing the candidate unit, Caddyfile, and Caddy drop-in. The helper itself is excluded from release transactions; changing it or the journal format requires a separate reviewed maintenance procedure while DSH is stopped. Valid abandoned siblings are removed under the deployment lock, while unsafe entries are retained for inspection without becoming recovery state. The helper reloads systemd, switches `current`, restarts DSH, verifies the PID owner, exact loopback-only `127.0.0.1:3080` listener, loopback login, public denial, and authenticated access, enables the service, syncs every affected `/opt`, `/etc`, and `/var` path, and only then records `committed`. Any interruption or failure before that commit restores and syncs the prior link, host files, process state, and enabled or disabled state; failed restoration retains the complete journal and blocks new work. A committed cleanup failure leaves state that the next operation removes without rollback. `DSH_HOME` does not roll back with code, so any future data migration requires a separate backward-compatibility assessment.
