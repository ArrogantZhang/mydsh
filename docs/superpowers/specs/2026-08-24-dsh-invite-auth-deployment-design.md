...
The server uses Node.js 24 and pnpm 11.7.0 with pinned tarball integrity. A `PrivateTmp` transient service runs clone, commit verification, frozen dependency installation, lifecycle scripts, tests, build, and config dump as `mydsh-build` under `env -i` with per-operation HOME, XDG directories including `XDG_RUNTIME_DIR`, private `TMPDIR`, cache, scratch `DSH_HOME`, and checkout. `KillMode=control-group` makes systemd kill and await every descendant before root reads a plain commit marker, validates it against the trusted commit without running Git in the builder-owned tree, and copies the completed tree without reflinks into new private inodes. The helper obtains privileged deployment inputs from a root-only trusted Git extraction. systemd then runs the accepted release as the separate non-login `mydsh` runtime user from `/srv/mydsh/workspace`; Caddy reads only the public environment file.
...
The root-installed helper builds a complete `prepared` journal in a root-only `activation.new.*` sibling containing the previous link, host-file backups, and prior service enablement, fsyncs it, and atomically renames it to `activation` before installing the candidate helper, unit, Caddyfile, and Caddy drop-in. Valid abandoned siblings are removed under the deployment lock, while unsafe entries are retained for inspection without becoming recovery state. The helper reloads systemd, switches `current`, restarts DSH, performs loopback, public, and authenticated acceptance, enables the service, and only then atomically records `committed`. Any interruption or failure before that commit restores the prior link, host files, process state, and enabled or disabled state; failed restoration retains the complete journal and blocks new work. A committed cleanup failure leaves state that the next operation removes without rollback. `DSH_HOME` does not roll back with code, so any future data migration requires a separate backward-compatibility assessment.
