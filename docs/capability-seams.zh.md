...
flowchart LR
  ...
  pkg_modules["modules"]
  pkg_hmr["hmr"]
  pkg_invite_auth["invite-auth"]
  svc_inviteAuthReadiness["ctx.inviteAuthReadiness<br/>Invite-auth route readiness"]
  pkg_web_app["web-app"]
  ...
  pkg_invite_auth --> svc_inviteAuthReadiness
  pkg_jobs --> svc_jobs
  ...
  svc_inviteAuthReadiness --> pkg_web_app
...
| `ctx.inviteAuthReadiness` | `core` | `invite-auth` | - | [`web-app`](../packages/bundle/web-app) | - | 认证 route 在发布就绪状态前已经存在；部署的 Web fallback 等待该状态，并在认证卸载时撤回。 |
