...
Choose **Download file** in the preview header to save the current original, including Word, Excel, PDF, and files without a preview renderer. The download reads through the same authenticated `workspaceFiles.readAll` method and its `maxFileBytes` limit; it never saves paginated text or a converted PDF in place of the source. Chinese filenames and original bytes are preserved. A refused read shows an error and starts no download. Browser handoff does not confirm a completed save. Closing the tab cancels pending reads; another download or tab closure releases its retained object URL.
