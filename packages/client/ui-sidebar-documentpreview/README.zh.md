...
点击预览头部的“下载文件”可保存当前原文件，包括 Word、Excel、PDF，以及没有预览器的文件。下载复用经过鉴权的 `workspaceFiles.readAll` 方法及其 `maxFileBytes` 限制，不会用分页文本或转换后的 PDF 替代源文件。中文文件名和原始字节保持不变。读取被拒绝时显示错误，不会开始下载。交给浏览器下载不代表保存已经完成。关闭 tab 会取消待完成读取；再次下载或关闭 tab 会释放保留的对象 URL。
