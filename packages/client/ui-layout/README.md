...
The root-scoped `shell.document.title` single slot owns the browser title when occupied. Its owner prop supplies `productTitle`; without an occupant, the stock Session-aware title component remains active. The frame never mounts both title writers together.
