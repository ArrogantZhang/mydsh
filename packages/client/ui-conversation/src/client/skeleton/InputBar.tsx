...
export const InputBar = memo(function InputBar({
  ...
  const submissionPending = useSession(s => s.pendingSubmissions.length > 0) ?? false
  ...
  const sending = machineBusy || submissionPending
  ...
  // An ordinary running session keeps Stop while the composer is empty,
  // owner-blocked, or submitting a command; an actionable draft gets Send, delivered
  ...
  const primaryStops = running && subagent === null && (empty || blocked !== undefined || machineBusy)
  const sendPending = sending && !primaryStops
      ...
      >
        ...
        <div className={css.row}>
          ...
          <div className={css.trailing}>
            ...
            <span className={css.visuallyHidden} aria-live="polite" role={sending ? 'status' : undefined}>{sending ? t('input.sending') : ''}</span>
            {interruptible && (
