...
`conversation.hero.content` is the session-maybe single slot for empty-conversation presentation. Its stock fallback remains the animated brand and headline. The workspace picker, agent preset, input dock, and native Composer stay outside this slot, so replacing the hero never remounts the editor.
...
Pending prompt admissions and command adjudication announce “Sending…” through a persistent status element; the Send button exposes `aria-busy` and a reduced-motion-aware marker. Optimistic prompt admission keeps the editor and subsequent sends available. Frozen command submissions keep Stop available for running Sessions.
