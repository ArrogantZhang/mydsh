...
describe('machine pending lock', () => {
  it('announces optimistic submissions without locking the next draft', () => {
    const { view, session, textarea, shell } = bench()
    act(() => {
      session.set(snapshotOf({ pendingSubmissions: [{
        requestId: 'pending' as never, placement: 'transcript', time: 1, text: 'first message', attachments: [],
      }] }))
    })
    expect(view.getByRole('status').textContent).toBe('发送中…')
    expect(view.getByRole('button', { name: '发送消息' }).getAttribute('aria-busy')).toBe('true')
    expect(editableOf(textarea)).toBe(true)
    act(() => { shell.setDraft('next message') })
    expect((view.getByRole('button', { name: '发送消息' }) as HTMLButtonElement).disabled).toBe(false)
  ...
  it.each(['Enter', 'button'] as const)('%s announces pending admission and restores a rejected draft', async (gesture) => {
    const deferred = Promise.withResolvers<SubmitOutcome>()
    const { view, textarea, shell } = bench({ draft: '/pending retry this' })
    act(() => {
      shell.beginCommand({ name: 'pending', token: '/pending ', submit: () => deferred.promise },
        { start: 0, end: 9, draftRev: shell.snapshot.draftRev })
    })
    const status = view.container.querySelector<HTMLElement>('span[aria-live="polite"]')!
    ...
    expect(editableOf(textarea)).toBe(false)
    await act(async () => { deferred.resolve({ kind: 'error' }); await deferred.promise })
    expect(status.textContent).toBe('')
    expect(primary.getAttribute('aria-busy')).toBeNull()
    expect(primary.disabled).toBe(false)
    expect(editableOf(textarea)).toBe(true)
    expect(shell.snapshot.draft).toBe('/pending retry this')
  })

  it('keeps Stop available during a running session submission', async () => {
    const deferred = Promise.withResolvers<SubmitOutcome>()
    const { view, textarea, stop, shell } = bench({ running: true, draft: '/pending follow up' })
    act(() => {
      shell.beginCommand({ name: 'pending', token: '/pending ', submit: () => deferred.promise },
        { start: 0, end: 9, draftRev: shell.snapshot.draftRev })
    })
    ...
    expect(stop).toHaveBeenCalledTimes(1)
    await act(async () => { deferred.resolve({ kind: 'success' }); await deferred.promise })
    expect(view.queryByRole('status')).toBeNull()
  })

  it('keeps a continuable child Stop usable while its command submission is pending', async () => {
    const deferred = Promise.withResolvers<SubmitOutcome>()
    const { view, textarea, stop, shell } = bench({
      running: true, draft: '/pending follow up',
      subagent: {
        address: { parentSessionId: 'parent' as SessionId, childSessionId: SID, mode: 'continuable' },
        parentAvailable: true,
      },
    ...
    expect(view.getByRole('status').textContent).toBe('发送中…')
    expect(view.getByRole('button', { name: '发送消息' }).getAttribute('aria-busy')).toBe('true')
    const interrupt = view.getByRole('button', { name: '停止生成' }) as HTMLButtonElement
