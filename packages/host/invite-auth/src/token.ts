 ...
 * SHA-256 first normalizes both UTF-8 inputs to fixed-length digests; only those equal-length digests are compared with timingSafeEqual.
...
export function verifySessionToken(token: string | undefined, secret: string, nowMs = Date.now()): boolean {
  ...
  if (
    ...
    providedSignature.length !== SIGNATURE_BYTES || providedSignature.toString('base64url') !== signatureText
  ) return false
