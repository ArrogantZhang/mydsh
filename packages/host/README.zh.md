...
`host/` 组提供 Web GUI 的普通 HTTP 服务器、服务已构建 Web 壳的 SPA dist 服务器、带原生／浏览／自适应组合包的工作区目录选择 seam、open-in-app 的应用探测与启动路由，以及只读的插件清单投影。这些都是产品包；浏览器传输位于 [`client/`](../client/README.zh.md)，组合应用是 [`apps/cli`](../../apps/cli/README.zh.md)，它启动 [`dsh-base` 组合包](../bundle/base/cordis.patch.yml) 来提供 `apps/web/` 下的 Web 应用。选择器后端可在共享 seam 后互相替换。
...
这些包提供 Host 能力；各包的 README 拥有自己的约定与配置。
...
| [`invite-auth/`](invite-auth/README.zh.md) | 邀请码登录及可选登录页展示 | `ctx.invitePage` |
| [`family-cover/`](family-cover/README.zh.md) | 经过鉴权的共享家庭照片存储 | `ctx.familyCover` |
