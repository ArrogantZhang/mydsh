...
The DSH Web server provides browser launch-token authentication but delegates TLS to a proxy. A small trusted group needs a shared invite-code entry while retaining the upstream browser session and carrier-level Host/Origin checks.
...
- Caddy is a required security component and port `3080` must remain private; direct access bypasses the invite layer; upstream browser authentication still applies.
