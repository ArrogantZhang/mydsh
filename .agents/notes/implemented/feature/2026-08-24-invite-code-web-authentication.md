...
The SSH-delivered SHA-256 sidecar detects artifact corruption but is not an authenticity proof. Deployment trusts the exact reviewed local ref and any separately verified signed tag or commit used before packaging. The host helper requires the commit-named artifact-set directory to contain exactly the archive and checksum, copies both into persistent root-private new inodes, enforces compressed, member-count, per-member, expanded-size, and free-space limits, rejects unsafe archive members and escaping links, and validates manifest format, commit, named ref, Linux amd64 platform, pinned image digest, runtime outputs, and helper-journal compatibility. Byte comparison against the installed control plane closes the candidate configuration grammar; the helper never executes release-contained control flow.
