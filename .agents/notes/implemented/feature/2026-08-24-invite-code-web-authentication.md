...
The Alibaba Cloud deployment installs reviewed root control flow at `/usr/local/sbin/mydsh-deploy-release`, outside every release. A shared nonblocking host lock serializes bootstrap, deployment, rollback, and pruning. Before mutation, the helper builds a complete `prepared` recovery journal in a root-only `activation.new.*` sibling containing the previous link, root-owned host-file backups, and prior service enablement, then atomically publishes it as `/var/lib/mydsh-deploy/activation`. Failure restores or replays that journal, including the previous enabled or disabled state, until recovery succeeds; accepted activation enables the service and records `committed` before cleanup, so cleanup failure can never roll back a live accepted release.

`mydsh-build` receives a new HOME, XDG config/cache/runtime directories, private `TMPDIR`, pnpm cache, scratch `DSH_HOME`, and checkout for each operation. One `PrivateTmp` transient service runs the complete build pipeline with `KillMode=control-group`; systemd kills and awaits all descendants before root validates the builder's plain commit marker against its trusted extraction and copies the result with reflinks disabled into new root-private inodes. Root never invokes Git in the builder-owned checkout. The builder cannot retain a writable inode or open descriptor into the published tree and cannot read runtime state, workspace files, the private environment file, or authentication secrets.
