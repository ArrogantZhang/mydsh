...
**使用 Caddy Basic Authentication。** Basic Authentication 会在每次请求中发送长期共享凭据，使部署难以控制退出登录和会话轮换，而且呈现浏览器原生提示框，而非产品的同源登录流程。短期签名会话把邀请码限制在登录 endpoint，并允许通过轮换签名秘密撤销所有浏览器会话。
