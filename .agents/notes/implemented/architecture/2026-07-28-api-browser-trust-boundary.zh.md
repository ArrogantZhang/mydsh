...
Web 组合通过 `dsh-host-webserver` 以纯 HTTP 提供 `/api`；该插件的配置在组合层接受 `127.0.0.1` 或 `0.0.0.0`。官方 `dsh web` CLI（命令行界面）拒绝 `--host 0.0.0.0` 并保持回环绑定；自定义组合自行负责所有非回环暴露。这个面上有远程代码执行级别的方法——`session.prompt` 驱动的 agent（智能体）可以运行 bash。浏览器会用两种经典方式把操作者变成攻击此类本地 API 的「混淆代理人」：恶意页面发出跨站「简单请求」 POST（`text/plain`——不经 CORS 预检即发出），其副作用照常执行、只是响应不可读；以及 DNS rebinding 后的源以「同源」身份直连 socket，CORS 整体失效，只有 `Host` 头会暴露攻击者的域名。在本决策之前，系统里唯一的浏览器信任检查（`isTrustedNativeDialogRequest`：回环 socket、同源、回环 Host）只守着一个装饰性的路由——`host.pickDirectory`，其原生对话框弹在宿主屏幕上——而所有真正具有严重后果的方法都没有防护。按 RPC 逐个设防也活不过应用内目录浏览器：它存在的意义就是服务合法的远程客户端，回环规则恰恰会拒绝它们。
...
两条边界仍有意留在本决策范围之外：可达性由 webserver 的绑定配置（`host: 127.0.0.1 | 0.0.0.0`）控制；认证属于独立的部署层。发布的 Web 组合默认保持无认证状态；远程部署可以选择启用[邀请码 Web 认证](../feature/2026-08-24-invite-code-web-authentication.zh.md)及其反向代理。本栅栏仍是混淆代理人防御，而不是认证层。旧守卫的回环 socket 检查被放弃而非泛化：绑定表达可达性、`trustedHosts` 点名远程权威之后，socket 地址提供不了头部栅栏覆盖不到的任何东西。
...
- 自定义非回环组合自行负责绑定，并且必须在 `trustedHosts` 中配置其对外服务 authority，否则请求会被拒绝。官方 `dsh web` CLI 保持回环绑定；其 `--trusted-host` flag 可以扩展可接受的 authority，但不会改变绑定。非浏览器自动化走同一道栅栏：回环地址或已声明的 authority 可通过，未声明的 DNS 别名会被拒绝。
