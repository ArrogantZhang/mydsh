...
浏览器真实载体为两类下行流各开一条独立 WebSocket：`/api/events.mux` 只发送 `MuxFrame`，`/api/events.host` 只发送 `HostFrame`。每条文本消息是一份完整的 `ServerRequest` JSON，或由[实时背压决策](../bug-fix/2026-08-27-web-realtime-backpressure.zh.md)定义的无损 `server-batch` wrapper；客户端会校验整条物理消息，再把每个窄形 `RpcRequest<Frame>` 交给既有 `ConnectionController`。两条流保持独立生命周期和无跨流顺序保证，任一条结束仍使整个 connection generation 失败并按既有退避策略重建。
