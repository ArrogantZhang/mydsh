...
Two boundaries stay deliberately out of scope: reachability is the webserver binding's policy (`host: 127.0.0.1 | 0.0.0.0`), and authentication belongs to a separate deployment layer. The shipped Web composition remains unauthenticated by default; remote deployments may opt into [invite-code Web authentication](../feature/2026-08-24-invite-code-web-authentication.md) and its reverse proxy. This fence remains a confused-deputy defense, not an authentication layer. The old guard's loopback-socket check was dropped rather than generalized: with binding expressing reachability and `trustedHosts` naming remote authorities, the socket address adds nothing a header fence does not already cover.
...
- Custom non-loopback compositions own their bind and must configure their serving authorities in `trustedHosts` or requests are refused. The official `dsh web` CLI remains loopback; its `--trusted-host` flag can extend authority acceptance without changing the bind. Non-browser automation rides the same fence: loopback or a declared authority passes, while an undeclared DNS alias is refused.
